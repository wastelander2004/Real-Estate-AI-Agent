    ...
    if target_url:
        ...
        if "失败" not in data and "异常" not in data and len(data) > 50:
            ...
            with st.spinner('🧠 正在生成专业研报...'):
                # 构建生成研报的专属 Prompt
                report_prompt = [
                    {"role": "system", "content": "你是一位顶级的宏观房地产金融分析师。请基于用户提供的数据输出包含趋势判断、市场情绪、投资组合建议三部分的快评。"},
                ...
                st.session_state.chat_history = [
                    {"role": "system", "content": f"你是一位房地产金融助教。请基于以下背景数据回答用户的问题。\n\n背景数据：{data[:1000]}"}
                ]
        else:
            st.error("❌ 获取失败，请检查网址。")
...
# --- 第二部分：多轮互动问答 ---
st.markdown("---")
st.header("2. 深度互动问答 (Q&A)")

# 只有在抓取数据后，才允许提问（确保 AI 有上下文）
if st.session_state.scraped_data:
    # 展示历史聊天记录 (跳过系统设定的背景设定)
